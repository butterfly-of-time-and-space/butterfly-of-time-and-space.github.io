...
(function () {
  ...
  var currentEditingWeaponSkillIndex = -1;
  ...
  function saveAllCharacters(allChars) {
    try {
      ...
      if (window.showToast) window.showToast('角色存档写入失败：' + (e && e.message ? e.message : ''), 'error');
      return false;
    }
  }
  ...
  function collectCharactersFromGrid() {
    ...
    cards.forEach(function (card, index) {
      ...
      // 关键：必须保留已有数据（包括 weapons、skills、gallery、story、profile、abilities 等所有字段），
      // 否则手动存档会清空用户在武器/技能等子页面编辑的内容。
      ...
      if (!merged.story) merged.story = { chapters: [] };
      if (!merged.profile) merged.profile = {};
      if (!merged.abilities) merged.abilities = DEFAULT_ABILITIES;
  ...
  function updateCurrentCharacter(patch) {
    if (!currentTimeline || currentEditingCharIndex < 0) {
      console.warn('[updateCurrentCharacter] 跳过：currentTimeline=', currentTimeline, 'currentEditingCharIndex=', currentEditingCharIndex);
      return false;
    }
    var chars = loadCharacters(currentTimeline);
    if (currentEditingCharIndex >= chars.length) {
      console.warn('[updateCurrentCharacter] 索引越界：index=', currentEditingCharIndex, 'chars.length=', chars.length);
      return false;
    }
    chars[currentEditingCharIndex] = Object.assign({}, chars[currentEditingCharIndex], patch);
    var ok = saveCharacters(currentTimeline, chars);
    if (!ok) {
      console.error('[updateCurrentCharacter] 保存失败，patch=', patch);
    }
    return ok !== false;
  }
  ...
  function addWeapon() {
    var char = getCurrentCharacter();
    if (!char) {
      console.error('[addWeapon] 当前角色为空，currentTimeline=', currentTimeline, 'currentEditingCharIndex=', currentEditingCharIndex);
      if (window.showToast) window.showToast('请先打开一个角色再创建武器', 'error');
      return;
    }

    var weapons = normalizeWeaponsData(char.weapons);
    weapons.push({ name: '', image: '', skills: [] });
    var saved = updateCurrentCharacter({ weapons: weapons });
    console.log('[addWeapon] 新增武器，timeline=', currentTimeline, 'index=', currentEditingCharIndex, 'weapons.length=', weapons.length, '保存结果=', saved);
  ...
  function createWeaponSkillCard(skill, index) {
    ...
    var plainDesc = skill.desc ? skill.desc.replace(/<[^>]+>/g, '') : '';
    descEl.textContent = plainDesc || '暂无描述……';
  ...
  function addWeaponSkill() {
    ...
    renderWeaponSkills(weapon.skills);
    // 自动打开新技能的独立编辑页
    openWeaponSkillEditor(weapon.skills.length - 1);
  }
  ...
  function initWeaponsPage() {
    ...
    // 刷新按钮：从 localStorage 强制重读，避免内存缓存问题
    var refreshBtn = document.getElementById('btn-refresh-weapons');
    if (refreshBtn) {
      refreshBtn.addEventListener('click', function () {
        if (!currentTimeline || currentEditingCharIndex < 0) {
          if (window.showToast) window.showToast('当前没有打开的角色', 'error');
          return;
        }
        var chars = loadCharacters(currentTimeline);
        var char = chars[currentEditingCharIndex];
        if (!char) {
          if (window.showToast) window.showToast('角色不存在', 'error');
          return;
        }
        var weapons = Array.isArray(char.weapons) ? char.weapons : [];
        if (window.showToast) {
          window.showToast('已重读本地存档 · 共 ' + weapons.length + ' 件武器', 'success');
        }
        console.log('[refresh-weapons] timeline=', currentTimeline, 'charIndex=', currentEditingCharIndex, 'weapons=', JSON.parse(JSON.stringify(weapons)));
        renderWeaponList(weapons);
      });
    }
  ...
  function initWeaponDetailBack() {
    ...
    var saveBtn = document.getElementById('btn-save-weapon-detail');
    if (saveBtn) {
      saveBtn.addEventListener('click', function () {
        saveWeaponDetail();
        if (window.OCStorage) window.OCStorage.save();
        if (window.showToast) window.showToast('武器资料已保存', 'success');
      });
    }
  }

  // ── 武器技能 · 独立编辑页 ────────────────────────────────────────
  function openWeaponSkillEditor(index) {
    if (!currentTimeline || currentEditingCharIndex < 0 || currentEditingWeaponIndex < 0) return;
    ...
    var weapons = normalizeWeaponsData(char.weapons);
    var weapon = weapons[currentEditingWeaponIndex];
    if (!weapon || !Array.isArray(weapon.skills)) return;

    var skill = weapon.skills[index];
    if (!skill) return;

    currentEditingWeaponSkillIndex = index;

    var pageTitle = document.getElementById('weapon-skill-edit-page-title');
    var titleEl = document.getElementById('weapon-skill-edit-title');
    var bodyEl = document.getElementById('weapon-skill-edit-body');

    if (pageTitle) pageTitle.textContent = (skill.name || '未命名技能') + ' · 武器技能编辑';
    if (titleEl) titleEl.innerHTML = skill.name || '';
    if (bodyEl) bodyEl.innerHTML = skill.desc || '';

    if (window.OCEdit && window.OCEdit.onWeaponSkillEditOpen) {
      window.OCEdit.onWeaponSkillEditOpen();
    }
  }

  function saveWeaponSkillEdit() {
    if (!currentTimeline || currentEditingCharIndex < 0 || currentEditingWeaponIndex < 0 || currentEditingWeaponSkillIndex < 0) return;
    ...
    var skill = weapon.skills[currentEditingWeaponSkillIndex];
    if (!skill) return;
  ...
  function initWeaponSkillEditPage() {
    var titleEl = document.getElementById('weapon-skill-edit-title');
    var bodyEl = document.getElementById('weapon-skill-edit-body');
    if (titleEl) titleEl.addEventListener('blur', saveWeaponSkillEdit);
    if (bodyEl) bodyEl.addEventListener('blur', saveWeaponSkillEdit);
  }

  function initWeaponSkillEditBack() {
    var btn = document.getElementById('btn-back-weapon-skill-edit');
    if (btn) {
      btn.addEventListener('click', function () {
        saveWeaponSkillEdit();
        if (window.OCStorage) window.OCStorage.save();
        openWeaponDetail(currentEditingWeaponIndex);
        if (window.OCEdit && window.OCEdit.onWeaponSkillEditBack) {
          window.OCEdit.onWeaponSkillEditBack();
        }
      });
    }
    var saveBtn = document.getElementById('btn-save-weapon-skill-edit');
    if (saveBtn) {
      saveBtn.addEventListener('click', function () {
        saveWeaponSkillEdit();
        if (window.OCStorage) window.OCStorage.save();
        if (window.showToast) window.showToast('武器技能已保存', 'success');
      });
    }
  }
  ...
  function initEditablePaste() {
    var editables = document.querySelectorAll(
      ...
      '#weapon-skill-edit-page [contenteditable="true"], ' +
  ...
  function init() {
    ...
    initWeaponSkillEditPage();
    initWeaponSkillEditBack();
    initSkillsPage();
    initSkillsBack();
    initSkillEditPage();
    initSkillEditBack();
  ...
  window.OCEdit = {
    ...
    onWeaponSkillEditOpen: null,     // -> weapon-skill-edit-page
    onWeaponSkillEditBack: null,     // -> character-weapon-detail-page
